...
export default function ContributionPage() {
  ...
  async function load() {
    ...
    const expTotal = (exp || []).reduce((sum: number, e: any) => sum + parseFloat(e.amount || 0), 0)
  ...
  function calcPerPerson() {
    if (totalExpenses > 0 && willingCount > 0) {
      return Math.round(totalExpenses / willingCount).toLocaleString('ar-SA')
    }
    return '—'
  }
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="text-2xl font-bold text-gray-900">
                {totalExpenses > 0 ? totalExpenses.toLocaleString('ar-SA') : '—'}
              </div>
            ...
            <div className="mb-5">
              ...
              <div className="space-y-2">
                {willingMembers.map((m, i) => {
                  const share = totalExpenses > 0 && willingCount > 0
                    ? Math.round(totalExpenses / willingCount).toLocaleString('ar-SA')
                    : null
                  return (
                    <div key={i} className="flex items-center justify-between bg-gray-50 rounded-lg px-4 py-2.5">
                      ...
                      <div>
                        {share ? (
                          <span className="text-sm font-bold text-emerald-600">{share} ريال</span>
                        ) : (
                          <span className="text-xs text-gray-400">سيُحسب بعد تسجيل المصروفات</span>
                        )}
                      </div>
                    </div>
                  )
          ...
          {totalExpenses > 0 && willingCount > 0 && (
            <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 text-sm text-emerald-700">
              {totalExpenses.toLocaleString('ar-SA')} ريال ÷ {willingCount} متطوع = <strong>{calcPerPerson()} ريال</strong> على كل مساهم
            </div>
          )}
        </div>
      )}
